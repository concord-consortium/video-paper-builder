/* Modified by Velir to support video time links */
...
var WysihatHelper = {
  ...
  faceboxVideoLink: function()
  {
      facebox.loading();
      new Effect.Appear($('facebox'), {duration: 0.3});
      iframe = this;
      facebox.reveal('Please enter video position in HH:MM:SS format: <input type=\"text\" id=\"link_field\" style=\"width:100%;\"/>', null);
      Event.observe('link_field', 'change', function(event) {
        iframe.videoLink($('link_field').value);
      });
  },
  ...
  editors.push(editor);
	...
	buttons.each(function(button){
		switch(button.toLowerCase()){
			...
			break;
			case 'videolink':
				toolbar.addButton({label : button.gsub('_','-').camelize().capitalize(), handler: function(editor) { return editor.faceboxVideoLink(editor); }});
			break;
			case 'html':
